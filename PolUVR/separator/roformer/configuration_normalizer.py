...
class ConfigurationNormalizer:
    ...
    def _normalize_single_value(self, key: str, value: Any, model_type: str) -> Any:
        ...
        if key in ["dim", "depth", "num_stems", "time_transformer_depth",
        ...
        # Float normalization
        if key in ["attn_dropout", "ff_dropout", "multi_stft_resolution_loss_weight",
                    "fmin", "fmax"]:
            if isinstance(value, str):
                try:
                    return float(value)
        ...
        if key.startswith("freqs_per_bands") or key in ["freqs_per_bands"]:
            if isinstance(value, str):
    ...
    def detect_model_type(self, config: dict[str, Any]) -> str | None:
        ...
        if isinstance(model_type, str):
            ...
            if "bs" in model_type_lower and "roformer" in model_type_lower:
                return "bs_roformer"
            if "mel" in model_type_lower and "roformer" in model_type_lower:
                return "mel_band_roformer"
            if "roformer" in model_type_lower:
